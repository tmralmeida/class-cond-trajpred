...
from ..deep_learning_based.deterministic.tf_modules import (
    PositionalEncoding,
    TransformerEncoder,
)
...
def get_noise(shape, noise_type):
    """retrieves noise vectors for deep generative models"""
...
class FFEncoder(nn.Module):
    """Feed forward based discriminator network"""
    ...
    def __init__(
        ...
        encoder_dims = copy(cfg["mlp_dims"])
        encoder_dims.insert(0, observation_len * (input_dims + self.class_emb_dim))
        self.classifier = make_mlp(
            encoder_dims, activation=cfg["activation"], dropout=cfg["dropout"]
        )
    ...
    def forward(self, x: Union[dict, torch.Tensor], **kwargs):
        ...
        if self.class_emb_dim > 0:
            ...
            inputs_cat = cat_class_emb(inputs_cat, labels)
        classifier_input = inputs_cat.contiguous().view(inputs_cat.size(0), -1)
...
class LinearConditionalMaskLogits(nn.Module):
    """runs activated logits through fc and masks out the appropriate discriminator score according
    to class number"""
